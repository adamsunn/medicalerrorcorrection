...
with open ("queryd.json") as f:
    data = json.load(f)
all_medical_sentences = []
# print(data)
for key, value in data.items():
    for sentence in value:
        # print(type(key), type(sentence), len(value))
        all_medical_sentences.append(sentence)
...
def add_repetition(line):
    line_list = line.split(" ")
    num_repetition_words = random.randint(1, max(1, int(len(line_list))))
...
def add_filler_words(line):
    ...
    for _ in range(num_filler_words):
        ...
        line_list = line.split(" ")
    print(f"add {num_filler_words} filler words")
    return line
...
agument_methods_1 = {'add': add_filler_words, 'rep': add_repetition, 'delete': delete_words, 'none': do_nothing}

agument_methods_2 = {'add': add_filler_words, 'rep': add_repetition, 'none': do_nothing, 'none': do_nothing}

agument_methods_3 = {'none': do_nothing, 'none': do_nothing, 'none': do_nothing, 'none': do_nothing}



def generate_json_data(all_conversational_lines, all_medical_sentences):
    data_dict = {}
    cur_agument_methods = agument_methods
...
for epoch in range(1, epoches+1):
    ...
    with open(os.path.join(cur_dir, f'augmented_video_{epoch}.json'), 'w') as convert_file:
