...
import time
import json
from jiwer import wer, RemovePunctuation
from model import *
...
if __name__ == '__main__':
    ...
    sample_num = 0
    preds = []
    data["source"] = [x if (x != "" and x != " ") else "<REMOVED>" for x in RemovePunctuation()(data["source"])]
    data["target"] = [x if (x != "" and x != " ") else "<REMOVED>" for x in RemovePunctuation()(data["target"])]
    bleu_scores = []
