...
def get_batched_examples(path_to_json, tokenizer, max_src_len, max_add_len, eval_split = 0.2):
    file_name = path_to_json.split("/")[-1]
    dir_name = "/".join(path_to_json.split("/")[:-1]) 
    json_files = [dir_name + "/" + pos_json for pos_json in os.listdir(dir_name) if pos_json.endswith('.json') and pos_json.startswith(file_name)]
    random.seed(0)
    random.shuffle(json_files)
    train = json_files[:int(len(json_files) * (1- eval_split))]
    evaluation = json_files[int(len(json_files) * (1 - eval_split)):]
    simple_tokenizer = SimpleTokenizer(" ")
    train_examples = {
        "src_token": [],
        "src_mask": [],
            ...
            for source, target in data_iter(examples_path):
                ...
                for idx in range(len(src)):
                    if tags[idx].startswith("CHANGE|"):
                        ...
                        target[idx].append(TAGS[tags[idx]])
                if (split == "train"):
                    train_examples["src_token"].append(src)
                    train_examples["src_mask"].append(src_mask)
                    train_examples["src_pos"].append(src_pos)
                    train_examples["target"].append(target)
                    train_examples["ASR"].append(ASR)
                    train_examples["RAW"].append(RAW)
                else:    
                    val_examples["src_token"].append(src)
                    val_examples["src_mask"].append(src_mask)
                    val_examples["src_pos"].append(src_pos)
                    val_examples["target"].append(target)
                    val_examples["ASR"].append(ASR)
                    val_examples["RAW"].append(RAW)
            print(f"Loaded {examples_path}")
